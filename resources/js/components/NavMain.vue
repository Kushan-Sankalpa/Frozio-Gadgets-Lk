...
const canViewItem = (item: NavItemWithSubmenu) => {
    const perms = userPermissions.value

    if (item.permission && !perms[item.permission]) {
        return false
    }
...
<template>
    <SidebarGroup class="px-2 py-0">
        ...
        <SidebarMenu>
            <Collapsible v-for="item in items.filter(canViewItem)" :key="item.title" class="group/collapsible"
