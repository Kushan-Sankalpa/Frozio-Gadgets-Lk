<template>
    <div>
        <Head title="Appointment list report" />

        <AppLayout :breadcrumbs="breadcrumbs">
            <!-- Export toast (same as summary report) -->
                    ...
                    >
                        <div>
                            ...
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                                View appointments/services by date range and branch.
                            </p>
                        </div>
  ...
  Apply filters
                        ...
                        <div class="grid gap-4 md:grid-cols-3">
                            ...
                            <div class="summary-card">
                                ...
                                <p class="value">{{ formatPercent(summary.avgValue) }}</p>
                            </div>
                        ...
                        <div class="mt-8">
                            ...
                            >
                                ...
                                <div v-if="rows.length" class="overflow-x-auto">
                                    <table class="min-w-[1600px] w-full text-sm border-separate border-spacing-y-2">
                                        <thead>
                                            <tr
                                                class="bg-zinc-50 text-xs font-semibold uppercase tracking-wide
                                                       text-zinc-500 dark:bg-zinc-900 dark:text-zinc-400"
                                            >
                                                <th class="py-2 pl-4 pr-4 text-left whitespace-nowrap">Appt. ref</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Client</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Contact no.</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Team member</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Status</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Scheduled date</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Scheduled time</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Service label</th>
                                                <th class="py-2 pr-4 text-right whitespace-nowrap">Duration</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Location</th>
                                                <th class="py-2 pr-4 text-right whitespace-nowrap">Net sales</th>
                                            </tr>
                                        </thead>
                                        ...
                                        <tbody>
                                            ...
                                            >
                                                ...
                                                <td class="py-2 pr-4 whitespace-nowrap">
                                                    <span :class="statusBadgeClass(r.payment_status)">
                                                ...
                                                <td class="py-2 pr-4 whitespace-nowrap text-right font-semibold text-zinc-900 dark:text-zinc-100">
                                                    {{ currencySymbol }} {{ formatNumber(r.net_sales) }}
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                ...
                                <p v-else class="px-4 py-4 text-sm text-zinc-500 dark:text-zinc-400">
                                    No appointment services found for this period.
                                </p>
                            </div>
                        </div>
                    </div>
...
export default {
    name: 'AppointmentListReportPage',
    ...
    data() {
        ...
        return {
            breadcrumbs: [
                ...
                { title: 'Appointment List', href: route('reports.appointments.list') },
            ] as BreadcrumbItem[],
    ...
    methods: {
        applyFilters() {
            ...
            ;(this as any).$inertia.get(route('reports.appointments.list'), this.filtersLocal, {
                preserveState: true,
                preserveScroll: true,
            })
        },
        ...
        async exportExcel() {
            ...
            const url = route('reports.appointments.list.excel', query)
            ...
            try {
                ...
                let filename = 'appointment-list-report.xlsx'
        ...
        async exportPdf() {
            ...
            const url = route('reports.appointments.list.pdf', query)
            ...
            try {
                ...
                const disposition = response.headers.get('Content-Disposition') || ''
                let filename = 'appointment-list-report.pdf'
                const match = disposition.match(/filename="?([^"]+)"?/i)
                if (match && match[1]) filename = decodeURIComponent(match[1])
        ...
        statusBadgeClass(status: string) {
            ...
            if (s === 'part_paid') {
                return `${base} bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-200`
            }
...
<style scoped>
/* Use Roobert font only inside this report page */
/* .report-font,
.report-font * {
