...
const loadedProducts = ref<FeaturedCosmetic[]>([])
...
function productHref(product: FeaturedCosmetic) {
...
async function fetchFeaturedCosmetics() {
  ...
  try {
    const response = await fetch('/home/featured-cosmetics', {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch featured cosmetics')
    }
    ...
    console.error('Featured cosmetics fetch error:', error)
...
<template>
  <section ref="sectionRef" class="mx-auto max-w-7xl px-3 py-8 sm:px-6 sm:py-12 lg:px-8">
    <div class="mb-5 flex items-end justify-between gap-3 sm:mb-7">
      <div>
        ...
        <p class="mt-1 text-sm text-neutral-500 sm:text-base">
          Selected featured cosmetic products from the latest collection.
        </p>
      </div>
    </div>
    ...
    >
      <h3 class="text-lg font-semibold text-red-700">Failed to load featured cosmetics</h3>
    ...
    >
      ...
      >
        <article
          class="cosmetic-product-card group overflow-hidden rounded-[20px] border border-neutral-200 bg-white shadow-sm sm:rounded-[24px]"
        >
          <div class="relative overflow-hidden bg-white">
            ...
            <div class="relative flex h-[180px] items-center justify-center px-3 pt-10 pb-3 sm:h-[240px] sm:px-4 sm:pt-12 sm:pb-4 xl:h-[260px]">
              <img
                ...
                :alt="product.name"
                class="cosmetic-main-image max-h-full max-w-full object-contain"
              ...
              <img
                ...
                class="cosmetic-hover-image max-h-full max-w-full object-contain"
              />
...
.cosmetic-product-card:hover .cosmetic-main-image {
  opacity: 0;
  transform: scale(1.01);
}

.cosmetic-product-card:hover .cosmetic-hover-image {
  opacity: 1;
  transform: scale(1);
}
