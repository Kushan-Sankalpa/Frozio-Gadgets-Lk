                    ...
                    >
                        <div>
                            ...
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                                View clients with engagement and loyalty info by appointment date range.
                            </p>
                        </div>
                        ...
                        <div class="mt-2">
                            ...
                            >
                                ...
                                <div v-if="rows.length" class="overflow-x-auto">
                                    <table class="min-w-[1500px] w-full text-sm border-separate border-spacing-y-2">
                                        <thead>
                                            <tr
                                                class="bg-zinc-50 text-xs font-semibold uppercase tracking-wide
                                                       text-zinc-500 dark:bg-zinc-900 dark:text-zinc-400"
                                            >
                                                <th class="py-2 pl-4 pr-4 text-left whitespace-nowrap">Client</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Contact no.</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Email</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Added on</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">First appt.</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Last appt.</th>
                                                <th class="py-2 pr-4 text-right whitespace-nowrap">Loyalty points</th>
                                                <th class="py-2 pr-4 text-left whitespace-nowrap">Loyalty tier</th>
                                            </tr>
                                        </thead>
                                        ...
                                        <tbody>
                                            ...
                                            >
                                                ...
                                                <td class="py-2 pr-4 whitespace-nowrap text-right font-semibold text-zinc-900 dark:text-zinc-100">
                                                    {{ Number(r.loyalty_points || 0).toLocaleString() }}
                                                </td>
...
export default {
    ...
    data() {
        ...
        return {
            breadcrumbs: [
                ...
                { title: 'Client List', href: route('reports.clients.list') },
            ] as BreadcrumbItem[],
            ...
            // DatePicker width fix: 2 calendars only on >= 1024, otherwise 1
            calendarCount: 2,
    ...
    methods: {
        applyFilters() {
            ...
            ;(this as any).$inertia.get(route('reports.clients.list'), this.filtersLocal, {
                preserveState: true,
                preserveScroll: true,
            })
        },
        ...
        async exportExcel() {
            ...
            const url = route('reports.clients.list.excel', query)
            ...
            try {
                ...
                let filename = 'client-list-report.xlsx'
        ...
        async exportPdf() {
            ...
            const url = route('reports.clients.list.pdf', query)
            ...
            try {
                ...
                const blob = await response.blob()
                const disposition = response.headers.get('Content-Disposition') || ''
                let filename = 'client-list-report.pdf'
                const match = disposition.match(/filename="?([^"]+)"?/i)
                if (match && match[1]) filename = decodeURIComponent(match[1])
...
<style scoped>
/* Keep the datepicker menu inside the container width (prevents overflow) */
:deep(.dp__menu) {
    max-width: 100%;
}
